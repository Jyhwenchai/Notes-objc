...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        // 类名
        NSLog(@"change before class name: %@", beforeCls);
...
//        [base addObserver:base forKeyPath:@"text" options:NSKeyValueObservingOptionOld|NSKeyValueObservingOptionNew context:nil];
        [base ilm_addObserver:base forKey:@"text" options:ILMKeyValueObservingOptionsOld|ILMKeyValueObservingOptionsNew];
        
        printf("============ 添加KVO之后 ============\n");

        Class afterCls = object_getClass(base);
        // 类名
        NSLog(@"change after class name: %@", afterCls);
